...
public class VisualSimulator_R extends JFrame implements VisualSimulator{
	...
	/* ----------------------------------------------------------------------------------
	* 설명 : 패널을 구성하는 각 컴포넌트들을 만들고 이를 패널에 붙여주는 메소드
	* 매개 : -
	* 반환 : -
	* ----------------------------------------------------------------------------------
	*/
	...
	/* ----------------------------------------------------------------------------------
	* 설명 : fileDialog를 통해 파일이 오픈되면 호출되며, SicSimulator의 인스턴스를 생성하고, update()함수를 통해서 GUI에 초기 정보를 업데이트한다.
	* 매개 : -
	* 반환 : -
	* ----------------------------------------------------------------------------------
	*/
	...
	public void initialize(File objFile, ResourceManager_R rMgr) {
		...
		firstInstruction.setText(rMgr.firstInst);
		startAddress2.setText(rMgr.startAddr_string[0]);
	...
	/* ----------------------------------------------------------------------------------
	* 설명 : oneStep 버튼에 매핑시켜놓은 메소드. SicSimulator의 oneStep() 메소드를 호출하며 호출 후 업데이트 된 정보를
	* 		GUI에 나타내준다
	* 매개 : -
	* 반환 : -
	* ----------------------------------------------------------------------------------
	*/
	...
	/* ----------------------------------------------------------------------------------
	* 설명 : allStep 버튼에 매핑시켜놓은 메소드. SicSimulator의 allStep() 메소드를 호출하며 호출 후 업데이트 된 정보를
	* 		GUI에 나타내준다
	* 매개 : -
	* 반환 : -
	* ----------------------------------------------------------------------------------
	*/
	...
	/* ----------------------------------------------------------------------------------
	* 설명 : SicSimulator에서 명령어를 수행하고 난 뒤, GUI에 수정된 정보를 뿌려주는 메소드.
	* 매개 : -
	* 반환 : -
	* ----------------------------------------------------------------------------------
	*/
	...
	public void update() {
		// TODO Auto-generated method stub
		programName.setText(rMgr.progName[rMgr.sectionIndex]);		//섹션 변화에 따른 프로그램 이름 업데이트
		startAddress.setText(rMgr.startAddr_string[rMgr.sectionIndex]);		//섹션 변화에 따른 프로그램 주소
//		programLength.setText(String.format("%06X", (rMgr.progLength[0]+rMgr.progLength[1]+rMgr.progLength[2]) / 2));
		programLength.setText(String.format("%06X", rMgr.progLength[rMgr.sectionIndex]/ 2)); //섹션 변화에 따른 프로그램 길이
		
		// 명령어 수행에 따른 각 레지스터 값 변화
		...
		currentDevice.setText(rMgr.currentDevice);	//Device에 접근할 때 변하는 CurrentUsingDevice를 업데이트해줌
		jList.setListData(rMgr.memoryField);	//명령어 수행에 따른 objectcode를 업데이트
		logList.setListData(rMgr.instField);	//명령어 수행에 따른 명령어 이름을 업데이트
	}
